...
public class VisaChecker {
	...
	private boolean checkPhoto(Applicant person) {
		  ...
		  if ((resolutionFirstMultiplier.equals(resolutionSecondMultiplier)) && ((360000 <= (multiplier1*multiplier2)) && (multiplier1*multiplier2) <= 1440000)) { 
	...
	private boolean checkPassport(Applicant person) {
		   ...
		   if (passportNum.length() == 10) {
			   ...
			   if (Character.isDigit(lastThird) && Character.isDigit(lastSecond) &&
					   Character.isDigit(lastFirst) && firstChar == 'P') {
	...
	private double computeExpirationDate(Applicant person) {
		   ...
		   // at the beginning we remove them before giving them to constructor of Date class
		   if (month.charAt(0) == '0') {
