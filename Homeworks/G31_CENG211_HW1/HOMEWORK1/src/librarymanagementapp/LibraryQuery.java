...
public class LibraryQuery {
	...
	private String mostIssuedMember(int[] array1, int[] array2, int[] array3, Member[] memberArray) {
		...
		// at the end of the for loop we will find how any times that the each member
		// has issued
	...
	public String leastIssuedComputerScienceMember(Issue[][] array1, Member[] memberArray) {
		...
		// returning the name of person who has issued least form computer science
		// library
	...
	public String fewestCopies(int row, int size, int entryNum, int copyAmount, Issue[][] array1, Book[] array2) {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < array1[row].length; j++) {
				if (array2[i + startPoint].getID().equals(array1[row][j].getBook())
						&& isContained(array2[i + startPoint], tempArray)) {
		...
		// As the first issue list contains two fewest amount of copy books we will
		// return the title of both
		...
		// returning the name of book object which has fewest copies and previosuly
		// issued
		index = 0;
		for (int i = 0; i < tempArray.length - copyAmount; i++) {
			if (tempArray[i].getQuantity() == minCopy) {
	...
	// These methods will call the other methods that we have defined so far to keep
	// the main program
	// simple (for clarity)
